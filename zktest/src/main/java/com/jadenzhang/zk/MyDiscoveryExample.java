...
public class MyDiscoveryExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            buildCache(client,serviceDiscovery);
    ...
    private static void buildCache(CuratorFramework client,ServiceDiscovery<InstanceDetails> serviceDiscovery) {
        CuratorCache cache = CuratorCache.build(client, PATH);
            CuratorCacheListener listener = CuratorCacheListener.builder()
                    .forCreates(node -> {
                        try {
                            int lengthPath=node.getPath().split("/").length;
                            int lengthPATH=PATH.split("/").length;
                            if (lengthPATH+LEVEL!=lengthPath) return;
                            JsonInstanceSerializer<InstanceDetails> js=new JsonInstanceSerializer<>(InstanceDetails.class);
                            ServiceInstance<InstanceDetails> details=js.deserialize(node.getData());
                            System.out.println(String.format("Node created: [%s]",
                                    details.getAddress()+details.getPort()));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    })
